...
# Expresions will be checked in direct order, one-by-one. Condition succceds when ALL results are True.
 ...
 NewWord = [ "OFF:(CTRL|ALT|META) SEQ:(((BACK)?SPACE):[01],)+((CAPS:[012],)|([LR]_SHIFT:[12],))*(@WORD@:1)", # "@WORD@:0" then collects the char
             "SEQ:(@WORD@:2,@WORD@:0)", # Drop repeated char at all: unlikely it needs correction
             "ON:(CTRL|L_ALT|META) SEQ:(,@WORD@:0)",
             "ON:(WORD) SEQ:(,@WORD@:1)", # New input after previous correction. "@WORD@:0" then collects the char
             "SEQ:(,([=-]|(KP)?MINUS|(KP)?ENTER|ESC|TAB):0)" ] # Be more flexible: chars line "-" can start new word, but must not completelly invalidate buffer!
 # Drop all buffers
